...
Run commands from this skill directory. Shared scripts live in `../findata-toolkit/`. (Map policy/macro releases to likely sector/style sensitivities and produce a monitoring checklist. Use when the user asks which sectors are sensitive to a policy change or wants a policy-impact brief.)
