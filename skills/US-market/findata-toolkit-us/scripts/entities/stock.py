...
class StockEntity:
    ...
    def get_full_report(self, stock_code: str, mode: str = "brief") -> dict:
        ...
        """
        # 1. 基础信息
        basic = self.client.fetch("us/company", {"stockCodes": [stock_code]})
        ...
        metrics_res = self.client.fetch("us/company/fundamental/non_financial", {
        ...
        if mode == "full":
            ...
            today_str = datetime.now().strftime("%Y-%m-%d")
            kline_res = self.client.fetch("us/company/candlestick", {
            ...
            # 增加高管变动
            executive_res = self.client.fetch("us/company/executive-shareholding", {
                "stockCodes": [stock_code],
                "limit": 5
            })
