...
Run commands from this skill directory. Shared scripts live in `../findata-toolkit/`. (Construct ETF allocations and quantify exposures (sector/style/factor), liquidity, and tracking considerations. Use when the user asks to build an ETF portfolio or analyze ETF exposures.)
...
| Script | Primary use |
| --- | --- |
| stock_data.py | ETF quotes / holdings proxy data available via yfinance fields when present. |

Notes:

- ETF constituents may be incomplete in yfinance for some tickers; consider extending toolkit if needed.
