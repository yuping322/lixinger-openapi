...
def get_stock_price(stock_code, start_date=None, end_date=None, adjust="qfq", limit=None):
    ...
    # API: hk/company/candlestick (or k-line)
    res = query_json("hk/company/candlestick", params)
...
def get_fundamentals(stock_code, date=None, metrics=None):
    ...
    res = query_json("hk/company/fundamental", params)
    
    if res['code'] == 1:
        df = pd.DataFrame(res['data'])
        return df
    else:
        print(f"Error fetching fundamentals for {stock_code}: {res}", file=sys.stderr)
        return None
...
def get_financials(stock_code, date=None, type="q", metrics=None):
    ...
    res = query_json("hk/company/financial", params)
...
if __name__ == "__main__":
    if init_lixinger():
        print(">>> Testing Price Data (00700 - Tencent)")
        df_price = get_stock_price("00700", start_date="2024-01-01", end_date="2024-01-10")
        if df_price is not None:
            print(df_price.head())
            print("\n✓ HK price data working successfully!")
            
        # Note: hk/company/fundamental endpoint returns "api not found"
        # This may be unavailable or require different parameters
        # Price data (candlestick) is working and sufficient for most use cases
        # print("\n>>> Testing Fundamentals (00700)")
        # df_fund = get_fundamentals("00700", date="2024-01-05")
