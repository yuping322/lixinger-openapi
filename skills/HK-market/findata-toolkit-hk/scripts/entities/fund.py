...
class FundEntity:
    ...
    def get_fund_report(self, fund_code: str) -> dict:
        ...
        # 1. 基金基础信息
        basic = self.client.fetch("cn/fund", {"stockCodes": [fund_code]})
        report["identity"] = basic.get('data', [{}])[0]

        # 2. 最新净值
        nav = self.client.fetch("cn/fund/net-value", {
        ...
        # 3. 基金持仓 (Top 10)
        holdings = self.client.fetch("cn/fund/holdings", {
            "stockCode": fund_code
        })
        report["top_holdings"] = holdings.get('data', [])[:10]

        # 4. 基金评级
        rating = self.client.fetch("cn/fund/rating", {
            "stockCodes": [fund_code]
        })
