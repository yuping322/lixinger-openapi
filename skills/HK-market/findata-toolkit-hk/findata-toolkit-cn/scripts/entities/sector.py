...
class SectorEntity:
    ...
    def get_sector_detail(self, sector_code: str) -> dict:
        ...
        # 1. 行业基本信息
        basic = self.client.fetch("hk/industry", {"stockCodes": [sector_code]})
        detail["identity"] = basic.get('data', [{}])[0]

        # 2. 行业估值 (PE/PB)
        valuation = self.client.fetch("hk/industry/valuation", {
        ...
        # 3. 行业成分股 (Top N)
        constituents = self.client.fetch("hk/industry/constituents", {
