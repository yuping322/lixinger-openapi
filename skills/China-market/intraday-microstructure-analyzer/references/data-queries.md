...
从本技能目录运行。共享数据脚本位于 `../findata-toolkit-cn/`。（分析分时/盘口/逐笔/分钟线与盘前数据，评估日内波动、量价结构与流动性风险。当用户询问分时、逐笔、盘口、分钟K、盘前/盘中结构或需要日内诊断时使用。）
...
| --- | --- | --- | --- | --- | --- |
| intraday_microstructure_dashboard | custom | composed view | 日内微观结构聚合：盘口/分时/分钟线（工具聚合视图）。 | required: symbol; common: date | data keys: bid_ask, intraday_em |
#### `intraday_microstructure_dashboard` 底层工具（plan 展开）


| key | tool | 说明 | 入参（必填/常用） | 关键输出字段（示例） |
| --- | --- | --- | --- | --- |
| bid_ask | stock_bid_ask_em | 东方财富-行情报价 | required: symbol | item, value |
| intraday_em | stock_intraday_em | 东方财富-分时数据 | required: symbol | 时间, 成交价, 手数, 买卖盘性质 |
