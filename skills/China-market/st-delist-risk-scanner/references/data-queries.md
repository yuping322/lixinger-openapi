...
从本技能目录运行。共享数据脚本位于 `../findata-toolkit-cn/`。（扫描风险警示板(ST)与退市/两网相关标的，输出风险标签、触发原因与可交易性约束。当用户询问ST、退市风险、两网及退市板块、或需要退市风险清单时使用。）
...
| --- | --- | --- | --- | --- | --- |
| st_delist_dashboard | custom | composed view | ST/退市相关标的与停复牌提醒（工具聚合视图）。 | - | data keys: risk_warning_board, stop_delist_board, staq_net_stop, trade_notify_suspend |
#### `st_delist_dashboard` 底层工具（plan 展开）


| key | tool | 说明 | 入参（必填/常用） | 关键输出字段（示例） |
| --- | --- | --- | --- | --- |
| risk_warning_board | stock_zh_a_st_em | 东方财富网-行情中心-沪深个股-风险警示板 | - | 序号, 代码, 最新价, 涨跌幅, 涨跌额, 成交量, 成交额, 振幅, 最高, 最低 |
| stop_delist_board | stock_zh_a_stop_em | 东方财富网-行情中心-沪深个股-两网及退市 | - | 序号, 代码, 最新价, 涨跌幅, 涨跌额, 成交量, 成交额, 振幅, 最高, 最低 |
| staq_net_stop | stock_staq_net_stop | 东方财富网-行情中心-沪深个股-两网及退市 | - | 序号, 代码 |
| trade_notify_suspend | news_trade_notify_suspend_baidu | 百度股市通-交易提醒-停复牌 | required: date | 股票代码, 股票简称, 交易所, 停牌时间, 复牌时间, 停牌事项说明 |
