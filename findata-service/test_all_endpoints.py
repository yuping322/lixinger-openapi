...
API_TESTS = [
    ...
    ("公司概况", "cn/company/profile", {
        "stockCodes": [TEST_SYMBOL]  # 使用 stockCodes 数组
    }),
    ...
    ("股东信息", "cn/company/shareholders", {
        "stockCode": TEST_SYMBOL,
        "startDate": start_date,
        "endDate": end_date
    }),
    
    ("高管增减持", "cn/company/executive-shareholding", {
        "stockCode": TEST_SYMBOL,
        "startDate": start_date,
        "endDate": end_date
    }),
    
    ("大股东增减持", "cn/company/major-shareholder-change", {
        "stockCode": TEST_SYMBOL,
        "startDate": start_date,
        "endDate": end_date
    }),
    ...
    # ========== 特殊数据 ==========
    ("龙虎榜", "cn/company/trading-abnormal", {
        "stockCode": TEST_SYMBOL,
        "startDate": start_date,
        "endDate": end_date
    }),
    
    ("大宗交易", "cn/company/block-trade", {
        "stockCode": TEST_SYMBOL,
        "startDate": start_date,
        "endDate": end_date
    }),
    
    ("股权质押", "cn/company/equity-pledge", {
        "stockCode": TEST_SYMBOL,
        "startDate": start_date,
        "endDate": end_date
    }),
    ...
    # ========== 财务相关 ==========
    ("分红送配", "cn/company/dividend", {
        "stockCode": TEST_SYMBOL,
        "startDate": start_date,
        "endDate": end_date
    }),
    
    ("营收构成", "cn/company/revenue-structure", {
        "stockCode": TEST_SYMBOL,
        "startDate": start_date,
        "endDate": end_date
    }),
    
    ("经营数据", "cn/company/operation-data", {
        "stockCode": TEST_SYMBOL,
        "startDate": start_date,
        "endDate": end_date
    }),
    ...
    # ========== 关系数据 ==========
    ("所属行业", "cn/company/related-industry", {
        "stockCode": TEST_SYMBOL
    }),
    
    ("所属指数", "cn/company/related-index", {
        "stockCode": TEST_SYMBOL
    }),
    
    # ========== 其他 ==========
    ("公告", "cn/company/announcement", {
        "stockCode": TEST_SYMBOL,
        "limit": 10
    }),
    
    ("热度数据", "cn/company/hot-data", {
        "stockCode": TEST_SYMBOL
    }),
    
    ("资金流向", "cn/company/fund-flow", {
        "stockCode": TEST_SYMBOL
    }),
    
    # ========== 监管信息 ==========
    ("监管信息", "cn/company/regulatory-info", {
        "stockCode": TEST_SYMBOL,
        "startDate": start_date,
        "endDate": end_date
    }),
    ...
    # ========== 客户供应商 ==========
    ("客户及供应商", "cn/company/client-supplier", {
        "stockCode": TEST_SYMBOL,
        "startDate": start_date,
        "endDate": end_date
    }),
...
def test_api(name: str, endpoint: str, params: dict) -> dict:
    ...
    try:
        ...
        return {
            ...
            "sample_data": data[0] if isinstance(data, list) and len(data) > 0 else (data if data else None)
        }
...
def main():
    ...
    for name, endpoint, params in API_TESTS:
        ...
        if result['success']:
            ...
            if result['has_data']:
                ...
                print(f"  ⚠️  成功 - 无数据 (code=1, data=[])")
        else:
            print(f"  ❌ 失败 - code={result['code']}, msg={result['message']}")
    ...
    # 保存详细结果
    output_file = Path(__file__).parent / "lixinger_api_comprehensive_test.json"
