...
def _read_evidence_ids(path: Path) -> Set[str]:
    ids: Set[str] = set()
...
def _check_bridge(adjustments: Dict[str, Any], verdict: Dict[str, Any], case_dir: Path) -> List[Issue]:
    issues: List[Issue] = []
...
def _check_scores(verdict: Dict[str, Any], case_dir: Path) -> List[Issue]:
    ...
    if not isinstance(scores, dict):
        issues.append(Issue(
            severity="Critical", location=loc,
    ...
    if overall is not None and grade is not None:
        if grade not in VALID_GRADES:
            ...
            expected_grade = (
                "A" if overall >= 0.80 else
                "B" if overall >= 0.65 else
                "C" if overall >= 0.45 else "D"
            )
...
def _check_red_flag_evidence_refs(red_flags_obj: Dict[str, Any], evidence_ids: Set[str], case_dir: Path) -> List[Issue]:
    issues: List[Issue] = []
...
def _check_cpe_files(case_dir: Path) -> Tuple[List[Issue], Dict[str, Optional[Path]]]:
    ...
    non_marker_files = [
        ...
        if f.is_file() and f.name != ".cpe_pending"
    ]
...
def _check_cpe_claims(claims_obj: Dict[str, Any], case_dir: Path) -> List[Issue]:
    ...
    for claim in claims:
        ...
        if confidence is None or not (0.0 <= float(confidence) <= 1.0):
            issues.append(Issue(
                severity="Warning", location=loc,
                description=f"claim {cid}: confidence={confidence} 超出 [0,1] 或缺失",
                impact="置信度字段异常",
        ...
        if not supporting:
            ...
            for i, ev in enumerate(supporting):
                ...
                if ev_conf is None or not (0.0 <= float(ev_conf) <= 1.0):
                    issues.append(Issue(
                        severity="Info", location=loc,
                        description=f"claim {cid} supporting_evidence[{i}]: confidence={ev_conf} 超出 [0,1] 或缺失",
                        impact="证据置信度字段异常",
                        suggested_fix="填写 [0,1] 范围内的 confidence",
                    ))
...
def _check_cpe_peer_clusters(peer_clusters_obj: Dict[str, Any], case_dir: Path) -> List[Issue]:
...
def _check_cpe_verdict(cpe_verdict_obj: Dict[str, Any], case_dir: Path) -> List[Issue]:
    ...
    if confidence is None or not (0.0 <= float(confidence) <= 1.0):
        issues.append(Issue(
            severity="Warning", location=loc,
            description=f"cpe verdict.confidence={confidence} 超出 [0,1] 或缺失",
            impact="置信度字段异常",
            suggested_fix="填写 [0,1] 范围内的 confidence",
        ))
...
def _check_subject_consistency(
    ...
    FQE financial_facts.company vs CPE verdict.subject.ticker
    """
    issues: List[Issue] = []
    ...
    if not fq_company or not cpe_ticker:
        return issues  # 字段缺失，由其他检查项报告

    if fq_company != cpe_ticker:
        issues.append(Issue(
            severity="Critical",
            location=str(case_dir),
            description=(
                f"FQE 主体（financial_facts.company={fq_company!r}）与 "
                f"CPE 主体（cpe verdict.subject.ticker={cpe_ticker!r}）不一致，"
                f"case 可能混拼了不同公司的数据"
            ),
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Deep Research QA: audit a research case directory outputs")
    ap.add_argument("--case", required=True, help="Case directory path, e.g. research_cases/case_YYYYMMDD_company")
    ap.add_argument(
        "--out",
        required=False,
    ...
    out_path = Path(args.out) if args.out else case_dir / "integrated" / "qa_report.json"
    ...
    if not fq_file_issues:
        ...
        evidence_ids = _read_evidence_ids(paths["normalized_evidence"])
        ...
        all_issues.extend(_check_red_flag_evidence_refs(red_flags_obj, evidence_ids, case_dir))
    ...
    if not args.skip_cpe:
        ...
        cpe_dir = case_dir / "competitive_positioning"
        if cpe_dir.exists() and cpe_paths.get("claims") and cpe_paths["claims"].exists():
            claims_obj = _read_json(cpe_paths["claims"])
            all_issues.extend(_check_cpe_claims(claims_obj, case_dir))

        if cpe_dir.exists() and cpe_paths.get("peer_clusters") and cpe_paths["peer_clusters"].exists():
        ...
        if cpe_dir.exists() and cpe_paths.get("cpe_verdict") and cpe_paths["cpe_verdict"].exists():
        ...
        if fq_verdict and cpe_verdict_obj:
            subject_issues = _check_subject_consistency(financial_facts, cpe_verdict_obj, case_dir)
            all_issues.extend(subject_issues)
            if not any(i.severity == "Critical" for i in subject_issues):
                all_issues.extend(_check_cpe_fqe_consistency(fq_verdict, cpe_verdict_obj, case_dir))
