...
2. **时间维度** — 默认：未来6–12个月
3. **行业分类** — 申万2021一级行业（31个，默认）、或更细的二级/三级行业
...
1. **宏观仪表盘** — 五大支柱当前状态及方向
2. **经济周期判断** — 当前阶段及位置
3. **行业信号表** — 全量申万2021一级行业（31个）的信号、逻辑、置信度
...
- 行业分类与行业信号默认基于理杏仁接口 `cn/industry/fundamental/sw_2021` 与 `cn/industry`。
- 一级行业覆盖口径固定为**申万2021一级行业 31 个**；若接口返回行业数量或名称与该口径不一致，必须在输出 `data_gaps` 中披露并下调置信度。
- 不与中信/Wind/GICS 行业分类进行混用或横向排名，除非用户明确要求并单独声明口径切换。
