...
本插件用于整合行业与概念板块研究能力，解决原先多个分散 skill 难以协同的问题。目标是把“行业表现、轮动判断、产业链验证、概念热度、政策映射、研报解构”打通成统一研究流水线。

### 1.1 设计目标

- **统一入口**：支持单点命令触发全链路研究。
- **模块解耦**：保留 5 个 skill 的独立性，便于单模块迭代。
...
- **Theme Engine（主题引擎）**：概念热度 + 政策敏感度 + 研报预期
